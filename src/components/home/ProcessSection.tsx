          ...
          {homeProcess.steps.map((step, i) => (
            <StaggerItem key={step.step}>
              <div className="relative h-full rounded-2xl border border-border bg-surface-card p-6 shadow-sm dark:shadow-none">
                <span className="font-[family-name:var(--font-poppins)] text-3xl font-bold text-valgor-500/25">
                  {step.step}
                </span>
                ...
                {i < homeProcess.steps.length - 1 && (
                  <span
                    className="absolute -bottom-3 left-1/2 hidden -translate-x-1/2 text-valgor-500 lg:block"
                    aria-hidden
                  >
                    ↓
                  </span>
