...
import { homeValgorScoreLead } from "@/lib/home-content";
...
export function ValgorScoreLeadSection() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`${VALGOR_SCORE_PATH}?${params.toString()}`);
  }

  return (
    <Section className="border-b border-border-subtle bg-gradient-to-b from-valgor-500/10 via-surface to-surface">
      <Container>
        <Reveal>
          <div className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-valgor-500/25 bg-surface-card shadow-xl">
            <div className="border-b border-border-subtle bg-valgor-500/5 px-6 py-8 text-center sm:px-10">
              <div className="inline-flex items-center gap-2 rounded-full bg-valgor-500/15 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-valgor-600 dark:text-valgor-400">
              ...
              <h2 className="mt-4 font-[family-name:var(--font-poppins)] text-2xl font-bold tracking-tight sm:text-3xl">
                {homeValgorScoreLead.title}
              </h2>
              <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-muted sm:text-base">
                {homeValgorScoreLead.subtitle}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 px-6 py-8 sm:px-10">
              ...
              <Button type="submit" className="w-full !rounded-xl sm:w-auto">
                {homeValgorScoreLead.cta}
              </Button>

              <p className="text-xs leading-relaxed text-muted">{homeValgorScoreLead.footnote}</p>
            </form>
          </div>
        </Reveal>
      </Container>
